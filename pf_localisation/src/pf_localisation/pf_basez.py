...
class PFLocaliserBase(object):
    ...
    def __init__(self):
        ...
        self.ODOM_TRANSLATION_NOISE = 0 	# Odometry x axis (forward) noiseupdate_lock
    ...
    def initialise_particle_cloud(self, initialpose):
        """
        Called whenever an initialpes message is received (to change the
        starting location of the roes), or a new occupancy_map is received.
        
        :Args:
            | initialpose: the inites pose estimate
        :Return:
    ...
    def predict_from_odometry(self, odom):
        ...
        with self._update_lock:
            ...
            if self.prev_odom_x == x and self.prev_odom_y == y and self.prev_odom_heading==new_heading:
                self.updated = False
                return time.time()-t
            ...
            for p in self.particlecloud.poses:
                ...
                newpose.poses.extend([p])
                print travel_x
                
            self.particlecloud = newpose
        return time.time() - t
